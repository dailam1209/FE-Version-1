...
import { addBuyProduct } from "../../../redux/cartbuyRedux";
...
import { ressetProduct } from "../../../redux/product/fetchProductApi";
import "swiper/css";
...
const Image = (props) => {
  ...
  const clickLike = (e) => {
    ...
    toast.success("Product had add to cart ao phong no dai qua", {
      height: "100%",
  ...
  const dispatchHandle = (id) => {
    ...
    console.log(dispatchProductToCart);
    if(dispatchProductToCart.src != "" && dispatchProductToCart.id != "" && dispatchProductToCart.color != "" ){
    ...
    <>
      <div onMouseLeave={() => handleDisplayNone()}>
        <div className="product-image-pos">
          <img class={`${props.image[0].code}img`} src={img} alt="" onClick={(e) => dispatchProductCode(e)}/>
          <button
